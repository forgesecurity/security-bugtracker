...
{  \usebackgroundtemplate{\includegraphics[width=1.0\paperwidth]{background.jpg}}
  ...
  \begin{TitleBox}
    ...
    Eric Therond \href{mailto: eric.therond.fr@gmail.com}{{\FA \faEnvelope}  eric.therond.fr@gmail.com}\\
    {\footnotesize \href{https://fr.linkedin.com/in/eric-therond-2b320ba1}{{\FA \faLinkedinSign} Eric Therond}
    \href{https://github.com/eric-therond/}{{\FA \faGithub} https://github.com/eric-therond/}
    }
   \end{TitleBox}
...
\begin{itemize}
\item Part of security tests is included into user acceptance tests
  \begin{itemize}
...
\begin{itemize}
\item Security tests coverage should include functionals tests
  \begin{itemize}
  \item What is the legitimacy to collect and process personal data
  \item Why so many clearances to this system
  \item Incidents which change security strategy
  \end{itemize}
\end{itemize}

\begin{itemize}
\item It's difficult to tests stakeholders and customers expectatives.
...
\begin{itemize}
\item Automating security testing is a \textbf{cost project} : you need very skilled and experimented QA for developping tools, documentation and process.
...
\item We are going to use open-source tools to test the exposure of organizations against thoses common vulnerabilities.
...
\item Automated security tests are the first attacks that a hacker will doing against your organization, your knowledge of theses techniques will help you to identify your attackers.
\end{itemize}
\end{block}
...
\item Those servers will be scan by dynamic analysis tools.
...
\item Those paths will be scan by static analysis tools.
...
\item When OpenVas finish a scan an alert is send to the bugtracker and detected bugs are recorded in the associated folder.
\end{itemize}
\end{block}
...
\item For example, each time a server is added or modified, a dynamic analysis scan can be run or each time a commit of code is done, a static analysis scan can be run.
\end{itemize}
\end{block}
